...
class YrApp extends Homey.App {
    ...
    async _initFlows() {
        ...
        this.homey.flow.getConditionCard('01_weather_next_hours')
            .registerRunListener((args, state) => args.device.nextHoursComparer(args, state,
                (ts: YrTimeserie, _value: number) => {
                    const symbolCode = requireConditionString(ts.data.next_1_hours?.summary.symbol_code, 'weather symbol');
        ...
        this.homey.flow.getConditionCard('02_measure_temperature.min_next_6_hours_below')
            .registerRunListener(args => capabilityIsBelow(args.device, 'measure_temperature.min_next_6_hours', args.value));

        this.homey.flow.getConditionCard('02_measure_temperature.max_next_6_hours_below')
            .registerRunListener(args => capabilityIsBelow(args.device, 'measure_temperature.max_next_6_hours', args.value));

        this.homey.flow.getConditionCard('02_temperature_next_hours_above')
            .registerRunListener((args, state) => args.device.nextHoursComparer(args, state,
                (ts: YrTimeserie, value: number) => requireConditionNumber(ts.data.instant.details.air_temperature, 'air temperature') > value))
            .getArgument('start')
        ...
        this.homey.flow.getConditionCard('03_rain_mm_next_hours_above')
            .registerRunListener((args, state) => args.device.nextHoursComparer(args, state,
                (ts: YrTimeserie, value: number) => requireConditionNumber(ts.data.next_1_hours?.details.precipitation_amount, 'precipitation amount') > value))
            .getArgument('start')
            .registerAutocompleteListener((query, args) => args.device.onTimeStartAutocomplete(query, args));
        ...
        this.homey.flow.getConditionCard('03_rain_probability_next_hours_above')
            .registerRunListener((args, state) => args.device.nextHoursComparer(args, state,
                (ts: YrTimeserie, value: number) => requireConditionNumber(ts.data.next_1_hours?.details.probability_of_precipitation, 'precipitation probability') > value))
            .getArgument('start')
            .registerAutocompleteListener((query, args) => args.device.onTimeStartAutocomplete(query, args));
        ...
        this.homey.flow.getConditionCard('03_rain_sum_next_hours_above')
            .registerRunListener((args, _state) => args.device.nextHoursSum(args,
                (ts: YrTimeserie) => requireConditionNumber(ts.data.next_1_hours?.details.precipitation_amount, 'precipitation amount'),
        ...
        this.homey.flow.getConditionCard('10_wind_strength_next_hours_above')
            .registerRunListener((args, state) => args.device.nextHoursComparer(args, state,
                (ts: YrTimeserie, value: number) => requireConditionNumber(ts.data.instant.details.wind_speed, 'wind speed') > value))
            .getArgument('start')
            .registerAutocompleteListener((query, args) => args.device.onTimeStartAutocomplete(query, args));
        ...
        this.homey.flow.getConditionCard('41_cloud_next_hours_above')
            .registerRunListener((args, state) => args.device.nextHoursComparer(args, state,
                (ts: YrTimeserie, value: number) => requireConditionNumber(ts.data.instant.details.cloud_area_fraction, 'cloud area fraction') > value))
            .getArgument('start')
            .registerAutocompleteListener((query, args) => args.device.onTimeStartAutocomplete(query, args));
        ...
        this.homey.flow.getConditionCard('51_fog_next_hours_above')
            .registerRunListener((args, state) => args.device.nextHoursComparer(args, state,
                (ts: YrTimeserie, value: number) => requireConditionNumber(ts.data.instant.details.fog_area_fraction, 'fog area fraction') > value))
            .getArgument('start')
            .registerAutocompleteListener((query, args) => args.device.onTimeStartAutocomplete(query, args));
        ...
        this.homey.flow.getConditionCard('60_thunder_next_hours_above')
            .registerRunListener((args, state) => args.device.nextHoursComparer(args, state,
                (ts: YrTimeserie, value: number) => requireConditionNumber(ts.data.next_1_hours?.details.probability_of_thunder, 'thunder probability') > value))
            .getArgument('start')
            .registerAutocompleteListener((query, args) => args.device.onTimeStartAutocomplete(query, args));
        ...
        this.homey.flow.getConditionCard('71_ultraviolet_next_hours_above')
            .registerRunListener((args, state) => args.device.nextHoursComparer(args, state,
                (ts: YrTimeserie, value: number) => requireConditionNumber(ts.data.instant.details.ultraviolet_index_clear_sky, 'ultraviolet index') > value))
            .getArgument('start')
            .registerAutocompleteListener((query, args) => args.device.onTimeStartAutocomplete(query, args));
