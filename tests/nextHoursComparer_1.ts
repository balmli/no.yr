        ...
        it('Check temperature above 10.1 starting 2022-07-26T13:00:00Z + 2 hours for 4 hours', function () {
            expect(
                nextHoursComparer(
                    ...
                    yrData1.properties.timeseries as YrTimeseries,
        ...
        it('Check rain (mm/h) above 1 starting 2022-08-02T12:00:00Z + 0 hours for 5 hours', function () {
            expect(
                nextHoursComparer(
                    ...
                    yrData2.properties.timeseries as YrTimeseries,
                    (ts: YrTimeserie, value: number) =>
                        !!ts.data.next_1_hours && (ts.data.next_1_hours.details.precipitation_amount as number) > value,
                ),
            ).eq(true);
        });
        ...
        it('Check weather situation starting 2022-08-02T12:00:00Z + 0 hours for 5 hours', function () {
            expect(
                nextHoursComparer(
                    ...
                    (ts: YrTimeserie, _value: number) =>
                        ...
                        (ts.data.next_1_hours.summary.symbol_code as string) === 'rainandthunder',
                ),
            ).eq(true);
            expect(
                nextHoursComparer(
                    ...
                    yrData2.properties.timeseries as YrTimeseries,
                    (ts: YrTimeserie, _value: number) =>
                        !!ts.data.next_1_hours && (ts.data.next_1_hours.summary.symbol_code as string) === 'lightrain',
                ),
            ).eq(false);
            expect(
                nextHoursComparer(
            ...
            expect(
                nextHoursComparer(
                    ...
                    yrData2.properties.timeseries as YrTimeseries,
                    (ts: YrTimeserie, _value: number) =>
                        !!ts.data.next_1_hours && (ts.data.next_1_hours.summary.symbol_code as string) === 'fog',
                ),
            ).eq(true);
            expect(
                nextHoursComparer(
                    ...
                    (ts: YrTimeserie, _value: number) => {
                        const symbolCode =
                            !!ts.data.next_1_hours && (ts.data.next_1_hours.summary.symbol_code as string);
