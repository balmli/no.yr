...
describe('forecast Flow conditions without data', () => {
    ...
    it('rejects comparer conditions so inversion cannot turn missing data into true', () => {
        expect(() => nextHoursComparer(undefined, nextArgs, undefined as any, () => true)).to.throw(
            WeatherDataUnavailableError,
        );
        expect(() => periodComparer(undefined, periodArgs, undefined as any, () => true)).to.throw(
            WeatherDataUnavailableError,
        );
    });
    ...
    it('rejects empty forecast arrays', () => {
        expect(() => nextHoursComparer(undefined, nextArgs, [], () => true)).to.throw(WeatherDataUnavailableError);
        expect(() =>
            nextHoursSum(
                undefined,
                nextArgs,
