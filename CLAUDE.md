...
This is a Homey app that provides weather forecasts from Yr.no (Norwegian Meteorological Institute). The app uses Homey SDK v3 and TypeScript, offering comprehensive weather data integration with 60+ flow cards for home automation.
...
**License Note:** This app uses weather data from Yr.no under their license: https://developer.yr.no/doc/License/
...
npx mocha -r ts-node/register tests/calculateFeelsLike.ts
```
...
- User-Agent header required: `no.yr/1.4.3 https://github.com/balmli/no.yr`
...
Per [Yr.no Terms of Service](https://developer.yr.no/doc/TermsOfService/):
...
- **User-Agent:** Required for all requests (currently: `YrAthomHomeyApp/{version} github.com/balmli/no.yr`)
