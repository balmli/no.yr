...
# Verify formatting without writing (also enforced by npm run lint)
...
Prettier is the source of truth for formatting (spacing, quotes, semicolons, line width, and
trailing commas). ESLint owns code-quality rules only. Run `npm run format` after editing
supported files and use `npm run lint` for the enforced formatting and code-quality gate.
...
3. **Period aggregations** - Use `periodSum()` for accumulation over time:
    ...
    ```typescript
    this.homey.flow.getConditionCard('03_rain_sum_period_above').registerRunListener((args, state) =>
